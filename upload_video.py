...
MEDIA_FORMATS = {"mp4": "video/mp4", "mov": "video/quicktime"}
...
def upload(photolib, photo_file, media_format):
    ...
    headers = {
        ...
        "X-Goog-Upload-Content-Type": f"{media_format}",
...
if __name__ == "__main__":
    ...
    delay = 5
    for photo_file_prefix in fetch_photos():
        files = glob.glob(f"{photo_file_prefix}.*")
        if len(files) == 0:
            print(f"Skip invalid format: {photo_file_prefix}")
            continue
